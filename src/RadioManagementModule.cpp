...
uint8_t RadioManagementModule::GetTxPowerIndex() const {
  /* Prefer the real per-unit calibrated index from EFUSE (what the hardware
   * actually uses), reported for the MCS7 reference rate on the current
   * channel/bandwidth. Falls back to the `power` member when EFUSE isn't
   * loaded yet (dongle not connected / autoload pending). */
  if (_eepromManager && _eepromManager->TxPowerInfoLoaded) {
    return _eepromManager->GetTxPowerIndexBase(
        /*path=*/0, /*rate MGN_MCS7=*/0x87, /*ntx_idx=*/0,
