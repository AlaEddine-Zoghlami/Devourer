...
public:
  ...
  /* Returns the dongle's real reference TX power index (0-63). When EFUSE
   * calibration is loaded, this is the per-unit calibrated value the driver
   * actually uses for MCS7 (path A, 1-stream) on the current channel/bw -- the
   * true "value from the device". Before EFUSE load it returns the `power`
